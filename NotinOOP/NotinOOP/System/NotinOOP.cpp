...
void NotinOOP::loadFragrances(std::ifstream& in, const std::string& firstToken)
{
    unsigned id = std::stoul(firstToken);

    double price, qty; 
    std::string name, brand, familyStr;
    size_t reviewsCount;
...
void NotinOOP::loadUsers(std::ifstream& in, const std::string& role)
{
    unsigned id; 
...
void NotinOOP::loadPurchases(std::ifstream& in, const std::string& firstToken)
{
    unsigned id = std::stoul(firstToken);
    unsigned userId;
    int statusInt; 
...
void NotinOOP::loadFromFile(const std::string& filename)
{
    ...
    while (in >> line) {
        ...
        if (currentSection == "#FRAGRANCES")  loadFragrances(in, line);
        else if (currentSection == "#USERS")  loadUsers(in, line);
        else if (currentSection == "#PURCHASES") loadPurchases(in, line);
    }
}
